...
async () => {
    const {data} = await axios.get("http://localhost/api/cars", 
    {headers: {
...
async (car) => {
    console.log(token)
    const {data} = await axios.post("http://localhost/api/cars", car, 
    {headers: {
        "Authorization" : `Bearer ${token}`
    }});
    return data
...
async (car) => {
    const {data} = await axios.put("http://localhost/api/cars", car, 
    {headers: {
        "Authorization" : `Bearer ${token}`
    }});
    return data
...
async (id) => {
    await axios.delete(`http://localhost/api/cars/${id}`, 
    {headers: {
        "Authorization" : `Bearer ${token}`
    }});
})
