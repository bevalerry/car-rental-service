...
async () => {
    const {data} = await axios.get("http://localhost/api/people",
    {headers: {
...
async (user) => {
    await axios.post("http://localhost/api/register", user);
    return user
})

export const fetchEditUser = createAsyncThunk("people/fetchEditUser",
async (user) => {
    const {data} = await axios.put("http://localhost/api/people", user, 
    {headers: {
        "Authorization" : `Bearer ${token}`
    }});
    return data
...
async (id) => {
    await axios.delete(`http://localhost/api/people/${id}`, 
    {headers: {
        "Authorization" : `Bearer ${token}`
    }});
})
